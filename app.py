...
# Połączenie z bazą danych Neo4j AuraDB
...
# Funkcja do pobierania danych z Neo4j
def run_query(query):
    try:
...
# Funkcja do dodawania Aircraft, Drone lub Soldier z UUID
def add_entity_with_uuid(entity_type, name):
    entity_uuid = str(uuid.uuid4())  # Generating unique UUID
...
# Funkcja do dodawania drona z kontrolą unikalności nazwy oraz dodaniem brandu
...
# Funkcja do generowania wykresu sieci
...
# Funkcja do usuwania drona, żołnierza lub samolotu
...
# Funkcja do przypisania żołnierza do drona
...
# Funkcja do wyświetlania żołnierzy i ich dronów
...
if option == Action.STATISTICS.value:
    ...
    brand = st.text_input("Enter Drone Brand")  # Input field for brand
...
# Zakończenie połączenia z Neo4j
driver.close()
